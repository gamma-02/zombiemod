...
public class ZombieSheep extends PathAwareEntity
{
    private final MeleeAttackGoal meleeAttackGoal = new MeleeAttackGoal(this, 1.5, false) {
        public void stop() {
            super.stop();
            ZombieSheep.this.setAttacking(false);
        }

        public void start() {
            super.start();
            ZombieSheep.this.setAttacking(true);
        }
    };
    public ZombieSheep(EntityType<? extends ZombieSheep> entityType, World world)
    {
    ...
    public static DefaultAttributeContainer.Builder createPigAttributes(){
        return HostileEntity.createHostileAttributes().add(EntityAttributes.GENERIC_FOLLOW_RANGE, 35.0D).add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.23D).add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 3.0D).add(EntityAttributes.GENERIC_ARMOR, 2.0D);
    }
    ...
    protected void damage(LivingEntity target) {
        if (this.isAlive()) {
            ...
            if (this.squaredDistanceTo(target) < 0.6D * (double)1 * 0.6D * (double)1 && this.canSee(target) && target.damage(
                ...
                this.playSound(SoundEvents.ENTITY_SHEEP_DEATH, 1.0F, (this.random.nextFloat() - this.random.nextFloat()) * 0.2F + 1.0F);
